...
## A. Project agreement — complete together on day 1
...
Chosen candidate:
Submission track, verified deadline, and source:

Concrete deployment example:
Research question:
Hypothesis:
Proposed contribution (still unverified):
Closest prior work and exact remaining difference:

Attacker objective:
Attacker knowledge:
Attacker control:
Attacker budget (queries / time / bytes / compromised components):
Trusted components:
Out-of-scope capabilities:

Primary outcome and denominator:
Benign utility and resource-cost metrics:
Strongest necessary baseline:
Smallest experiment:
What result would make us stop:
What observation would indicate a bug:

Available hardware / datasets / checkpoints:
Missing prerequisites, owner, and resolution date:
Estimated pilot compute cost:
...
|---|---|---|---|---|
| <reviewer's concrete concern> | <consequence> | <evidence/path or planned test> | <change> | No |
